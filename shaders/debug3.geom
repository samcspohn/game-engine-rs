...
layout(set = 0, binding = 0) buffer d { DrawAABB aabbs[]; };
...
void draw_line(in mat4 vp, vec3 a, vec3 b, vec4 color, float _width) {
    ...
    vec4 _b = vp * vec4(b, 1);
    vec4 z = vec4(0, 0, 1, 1);
    vec3 width_dir = cross(normalize(_a.xyz - _b.xyz), z.xyz);
...
#define p     particles[i]
    ...
    mat4 vp = proj * view;
    DrawAABB a = aabbs[_i];
    // DrawArrow ar = arrows[_i];
    // vec3 end_point = ar.pos + ar.dir * ar.size;
    // // vec3 cam_pos = (view * vec4(0,0,0,1)).xyz;
    // vec3 axis = normalize(end_point - cam_pos);
    // axis = normalize(cross(axis, normalize(ar.dir)));
    // axis = normalize(cross(axis, normalize(ar.dir)));
    ...
    vec4 color = a.color;
    draw_line(vp, corners[0], corners[1], color, a.size);   // left side
    draw_line(vp, corners[1], corners[3], color, a.size);
    draw_line(vp, corners[3], corners[2], color, a.size);
    draw_line(vp, corners[0], corners[2], color, a.size);

    draw_line(vp, corners[4], corners[5], color, a.size);   // right side
    draw_line(vp, corners[5], corners[7], color, a.size);
    draw_line(vp, corners[7], corners[6], color, a.size);
    draw_line(vp, corners[4], corners[6], color, a.size);

    draw_line(vp, corners[0], corners[4], color, a.size);   // left to right
    draw_line(vp, corners[1], corners[5], color, a.size);
    draw_line(vp, corners[2], corners[6], color, a.size);
    draw_line(vp, corners[3], corners[7], color, a.size);

    // draw_line(vp, ar.pos, end_point, ar.color, 0.02);
    // vec3 arrow_dir = normalize(rotate(axis, radians(45), ar.dir * ar.size));
    // draw_line(vp, end_point, end_point - arrow_dir, ar.color, 0.02);
    // arrow_dir = normalize(rotate(axis, radians(-45), ar.dir * ar.size));
    // draw_line(vp, end_point, end_point - arrow_dir, ar.color, 0.02);
