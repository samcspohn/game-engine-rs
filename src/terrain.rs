...
use {
    // crate::model::{Normal, Vertex, UV},
    // crate::renderer_component::buffer_usage_all,
    // crate::texture::Texture,
    force_send_sync::SendSync,
...
struct TerrainChunkRenderData {
    ...
    pub intance_buffer: Arc<CpuAccessibleBuffer<[i32]>>,
}
// #[component]
    ...
    pub fn generate(&mut self, transform: &Transform, sys: &System) {
        ...
        if self.tcrd.is_none() {
            ...
            self.tcrd = Some(Arc::new(TerrainChunkRenderData {
                ...
                intance_buffer: CpuAccessibleBuffer::<[i32]>::from_iter(
                    &sys.vk.mem_alloc,
                    BufferUsage {
                        transfer_src: true,
                        storage_buffer: true,
...
impl Component for Terrain {
    fn on_render(&mut self, t_id: i32) -> Box<dyn Fn(&mut RenderJobData)> {
        ...
        if let Some(tcrd) = &self.tcrd {
            ...
            let instance_buf = tcrd.intance_buffer.clone();
            Box::new(move |rd: &mut RenderJobData| {
                ...
                descriptors.push(WriteDescriptorSet::buffer(2, index_buffer.clone()));
